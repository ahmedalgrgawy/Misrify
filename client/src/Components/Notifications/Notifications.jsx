    ...
    return (
        <div className="p-4 md:p-10 lg:px-16 bg-bg-second flex flex-col items-center">
            <div className="flex justify-between items-center p-6 w-full max-w-4xl">
                <h3 className="text-3xl font-bold text-title-blue">Notifications</h3>
                <button
            ...
            <div className="flex flex-col gap-5 p-6 w-full max-w-4xl">
                {notifications.map(({ _id, sender, content, isRead, createdAt }) => (
                    <div key={_id} className="p-6 bg-white rounded-lg shadow-md transition ease-in hover:shadow-lg duration-400 border border-gray-100 ">
                        <div className="flex gap-4 items-center mb-4">
                            <h4 className="font-semibold text-xl text-main-blue">{getSenderName(sender)}</h4>
                            <p className="text-sm text-dark-grey">{new Date(createdAt).toLocaleString()}</p>
                        </div>
